...
func (p *ProcUptime) Provides(pr pkg.Provides) { pr["run.uptime"] = p }

// Requires declares file dependency.
func (p *ProcUptime) Requires(r *pkg.Requires) { r.Files = append(r.Files, "/proc/uptime") }

// Parse extracts uptime seconds and formats via SecondsToText. Failure yields ParseFailMsg.
func (p *ProcUptime) Parse(f *pkg.Facts) {
